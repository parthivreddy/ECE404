...
def encryption(plaintextFile, roundKeyList):
    BLOCKSIZE = 64
    ...
    cipherText = encrypted_bv.get_bitvector_in_hex()
    #print(cipherText)
    f = open(sys.argv[4], 'w')
    f.write(cipherText)
    f.close()
...
def main():
    ...
    if(sys.argv[1] == "-e"):
        #call encryption
        encryption(sys.argv[2], roundKeyList)
    elif(sys.argv[1] == "-d"):
        reverseKeys = [x for x in roundKeyList[::-1]]
        decryption(sys.argv[2],reverseKeys)
    else:
        #print("Error")
        sys.exit(1)
