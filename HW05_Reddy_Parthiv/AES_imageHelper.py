...
def encryption(plaintext_bv, keyWords, outFile, iv, subTable, header_bv): #implements AES encryption
    AES_modulus = BitVector(bitstring='100011011')
    ...
    for i in range(numBlocks): #do for each 128 bit block
        currBlockXOR = plaintext_bv[i*BLOCKSIZE : (i+1) * BLOCKSIZE]
        currBlock = iv
        ...
        afterXOR = currBlockXOR ^ stepFour_bv
        afterXOR.write_to_file(FILEOUT)
        iv_int = iv.int_val()
        iv_int+=1
